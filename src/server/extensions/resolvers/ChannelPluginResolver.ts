...
import fs from 'fs';
import { BasePlugin, type PluginConfirmHandler, type PluginMessageHandler } from '@server/channels/plugins/BasePlugin';
...
function createDuckTypedWrapper(
  ...
  return class ExtensionDuckTypedWrapper extends BasePlugin {
    ...
    protected async onInitialize(config: import('@server/channels/types').IChannelPluginConfig): Promise<void> {
    ...
    async sendMessage(
      chatId: string,
      message: import('@server/channels/types').IUnifiedOutgoingMessage
    ): Promise<string> {
