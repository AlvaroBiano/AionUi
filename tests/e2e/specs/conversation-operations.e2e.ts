 ...
 * 策略：
 *   1. 强制通过 DOM 移除所有 Arco modal portal 相关元素（wrapper + mask）
 *   2. 导航到 /guid 并等待稳定
 */
async function dismissAndNavigateToGuid(page: import('@playwright/test').Page): Promise<void> {
  // 强制移除所有 Arco modal portal（包括 mask），不发送 Escape 避免触发全局快捷键副作用
  await page.evaluate(() => {
    // arco-modal-wrapper 包含 modal 内容，arco-modal-mask 是遮罩层
    // 两者都是 React portal，不随 hash 切换自动卸载
    const selectors = ['.arco-modal-wrapper', '.arco-modal-container', '[class*="arco-modal-mask"]'];
    for (const sel of selectors) {
      document.querySelectorAll(sel).forEach((el) => el.remove());
    }
    // 同时移除 body 上的 overflow:hidden（Arco modal 打开时设置）
...
test.describe('AC11–AC12: 重命名', () => {
  ...
  test('AC11: 点击"重命名"弹出含输入框的模态框，输入框预填当前名称', async ({ page }) => {
    ...
    const modal = page.locator(`${ARCO_MODAL}, ${ARCO_MODAL_CONFIRM}`).first();
    await expect(modal, 'AC11: 重命名模态框应出现').toBeVisible({ timeout: 5_000 });
  ...
  test('AC12: 重命名弹窗有取消按钮，点击后弹窗关闭名称不变', async ({ page }) => {
    ...
    await cancelBtn.click();
    await expect(modal, 'AC12: 弹窗应关闭').toBeHidden({ timeout: 3_000 });
...
test.describe('AC18: 删除确认后从侧边栏移除；当前查看该会话时跳转回 /guid', () => {
  test('AC18: 删除当前会话后页面跳转回 /guid', async ({ page }) => {
    ...
    // 右键菜单 → 删除
    const menu = await openContextMenu(page, _deleteConvId!);
    await clickMenuItemByText(page, menu, /delete|删除/i);
