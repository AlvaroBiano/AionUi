...
test.describe('AC13–AC14: 删除确认框', () => {
  ...
  test('AC13: 点击"删除"弹出包含取消/确认按钮的确认框', async ({ page }) => {
    ...
    // Modal.confirm() 创建新的 modal wrapper；通过 wrapper 过滤出可见的 modal
...
test.describe('AC18: 删除确认后从侧边栏移除；当前查看该会话时跳转回 /guid', () => {
  test('AC18: 删除当前会话后页面跳转回 /guid', async ({ page }) => {
    ...
    // 触发删除对话框
    // 在 conversation 页面使用 evaluate 直接调用 handleDeleteClick（通过 Modal.confirm 路径）
    // 通过模拟点击 3-dot button（它使用 onClick → onOpenMenu，不是 contextmenu）
    const row = page.locator(`#c-${_deleteConvId!}`);
    await row.waitFor({ state: 'visible', timeout: 8_000 });

    // 使用 evaluate 模拟 hover（触发 CSS group-hover），再点击 3-dot span
    await page.evaluate((convId) => {
      const rowEl = document.getElementById(`c-${convId}`);
      if (!rowEl) return;
      // 找到 3-dot button（最后一个 flex-center cursor-pointer span）
      const btns = rowEl.querySelectorAll('.flex-center.cursor-pointer');
      const threeDot = btns[btns.length - 1] as HTMLElement | undefined;
      if (threeDot) {
        threeDot.style.display = 'flex'; // 强制显示
        threeDot.click();
      } else {
        // fallback: contextmenu event
    ...
    if (!menuVisible) {
      test.skip(true, 'AC18: 无法在 conversation 页面打开操作菜单（dropdown 不可见）');
      return;
    }
