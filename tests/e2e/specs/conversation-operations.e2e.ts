 ...
 * 策略：
 *   1. 等待 modal 自然关闭（各个测试已显式关闭弹窗）
 *   2. 仅当还有残留 modal-mask 拦截时，强制 DOM 移除（不影响 React 状态）
 *   3. 导航到 /guid 并等待稳定
 *
 * 注意：不要在 modal 动画播放中途强制移除 DOM，会导致 React fiber 状态损坏。
 */
async function dismissAndNavigateToGuid(page: import('@playwright/test').Page): Promise<void> {
  // 先等最多 500ms 让 modal 关闭动画完成（cancel/Escape 后约 200ms 动画）
  await page.waitForFunction(() => !document.querySelector('.arco-modal-wrapper'), { timeout: 500 }).catch(() => {
    // 超时说明有残留 modal，此时强制移除（保护 body overflow 状态）
    return page.evaluate(() => {
      document.querySelectorAll('.arco-modal-wrapper, .arco-modal-container, [class*="arco-modal-mask"]').forEach(
        (el) => el.remove()
      );
...
test.describe('AC18: 删除确认后从侧边栏移除；当前查看该会话时跳转回 /guid', () => {
  test('AC18: 删除当前会话后页面跳转回 /guid', async ({ page }) => {
    ...
    // 通过 IPC 直接删除当前会话（等价于确认删除操作的效果）
    // 这测试的是"当前会话被删除时 → 自动跳转回 /guid"的路由行为
    await invokeBridge(page, 'remove-conversation', { id: _deleteConvId! });
