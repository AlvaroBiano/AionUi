...
vi.mock('@server/http/auth/service/AuthService', () => ({
  AuthService: {
    verifyToken: vi.fn(),
...
describe('extractWebSocketToken – cookie parsing with special characters', () => {
  let TokenMiddleware: typeof import('@server/http/auth/middleware/TokenMiddleware').TokenMiddleware;

  beforeEach(async () => {
    vi.resetModules();
    const mod = await import('@server/http/auth/middleware/TokenMiddleware');
