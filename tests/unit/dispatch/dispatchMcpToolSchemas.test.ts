...
describe('F-5.1: MCP Tool Schema Sync', () => {
  ...
  // MCP-TS-001: All 4 tools are present
  describe('MCP-TS-001: TOOL_SCHEMAS has all 4 tools', () => {
    it('contains exactly start_task, read_transcript, list_sessions, send_message', () => {
      const names = schemas.map((s) => s.name).sort();
      expect(names).toEqual(['list_sessions', 'read_transcript', 'send_message', 'start_task']);
    });
  });
