...
- 在线程底部、输入框上方出现一条 warmup row：
  - 在还没有 live 激活证据前，副标题是 `Connecting to {agent}`
  - 一旦 ACP 已经 `connected / authenticated / session_active`，但首个可见响应还没到，副标题会切成 `Waiting for the first response from {agent}`
